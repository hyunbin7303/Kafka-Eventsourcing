    ...
    public class RestoreReadDbController : ControllerBase
    {
        ...
        public async Task<ActionResult> RestoreReadDbAsync()
        {
            try
            {
                ...
                {
                    Message = "Read database restore request completely successfully."
                }); 
            ...
            {
                const string SAFE_ERR_MSG = "Error while processing request to restore read db.";
                _logger.Log(LogLevel.Error, ex, SAFE_ERR_MSG);
                return StatusCode(StatusCodes.Status500InternalServerError, new BaseResponse
                {
                    Message = SAFE_ERR_MSG
                });
            }
        }
    }
